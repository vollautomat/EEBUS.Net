...
namespace EEBUS.Controllers
{
	public class BrowserController : Controller
	{
		...
		public IActionResult Index()
		{
			try
			{
				return View( "Index", new ServerNodes() { Nodes = getEEBUSNodes(), LocalSKI = model.LocalSKI } );
			}
			catch (Exception ex)
			{
		...
		public async Task<IActionResult> Accept()
		{
			try
			{
				...
				if ( wsClient.State == WebSocketState.Open )
				{
					...
					Client client = new Client( hostString, wsClient, this.devices, this.devices.GetRemote( model.Id ) );
